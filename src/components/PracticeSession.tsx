  ...
  return (
    <div className="min-h-screen bg-black text-right pb-40" dir="rtl">
      <div className="max-w-6xl mx-auto px-6 py-10 space-y-12">
        <div className="flex justify-between items-center glass-card p-6 rounded-[35px] sticky top-6 z-50 border-white/10 backdrop-blur-3xl shadow-2xl">
          <Button onClick={onExit} variant="ghost" className="text-xl font-black text-white/40"><ChevronLeft className="ml-2" /> انسحاب</Button>
          {mode !== 'normal' && (
            <div className={cn("px-10 py-3 rounded-full border-2 font-mono text-4xl font-black shadow-inner transition-all", timeLeft < 20 ? "border-rose-500 text-rose-500 animate-pulse shadow-rose-500/20" : "border-white/10 text-white shadow-white/5")}>
              {Math.floor(timeLeft/60)}:{(timeLeft%60).toString().padStart(2, '0')}
            </div>
          )}
          <div className="text-2xl font-black text-primary drop-shadow-glow">{currentQuestionIndex + 1} / {section.questions.length}</div>
        </div>

        <Progress value={progress} className="h-4 rounded-full bg-white/5 border border-white/5" />
        ...
        <Card className="p-12 glass-card rounded-[60px] border-white/10 space-y-12 relative overflow-hidden shadow-2xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {q.options.map((opt, i) => (
              <button key={i} onClick={() => handleAnswer(opt)} className={cn("p-10 rounded-[40px] text-right text-3xl font-black transition-all border-2 relative overflow-hidden group", userAnswers[q.id] === opt ? "bg-primary text-white border-white scale-[1.02] shadow-2xl" : "bg-white/5 text-white border-white/5 hover:border-primary/40 active:scale-95")}>
                <span className="opacity-30 ml-6 font-bold">{['أ', 'ب', 'ج', 'د'][i]}.</span> {opt}
        ...
        <div className="flex justify-center md:justify-end">
          <Button onClick={handleNext} className="w-full md:w-auto px-24 h-28 text-5xl font-black bg-primary rounded-[40px] shadow-2xl active:scale-95 transition-all shadow-primary/20">
