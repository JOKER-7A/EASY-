...
export default function PracticeSession({ section, onExit }: PracticeSessionProps) {
  ...
  const handleNext = useCallback(() => {
    ...
    if (lastIndexInGroup < section.questions.length - 1) {
      ...
      if (isPressureMode) setTimeLeft(60); // Reset for pressure mode
      ...
      <div className="fixed inset-0 z-[100] flex items-center justify-center bg-midnight/95 backdrop-blur-3xl animate-in fade-in duration-1000">
        <div className="text-center space-y-12 px-6">
          <div className="relative">
            ...
            <h2 className="text-6xl md:text-8xl font-headline font-black text-white drop-shadow-[0_0_30px_rgba(255,255,255,0.4)]">
              أهم شيء الفهم وليس الحفظ 💡
            </h2>
          </div>
          <p className="text-2xl md:text-3xl font-bold text-goldenrod/80 tracking-widest animate-pulse">
            جاري تحضير بيئة الاختبار...
          </p>
        </div>
      </div>
    );
      ...
      <div className="min-h-screen flex items-center justify-center p-4 bg-midnight animate-in fade-in slide-in-from-bottom-12 duration-700">
        <div className="max-w-6xl w-full space-y-16">
          <div className="text-center space-y-4">
            <h1 className="text-7xl font-black text-white">اختر وضع التحدي 🎮</h1>
            <p className="text-2xl text-muted-foreground font-bold italic opacity-70">"حدد الطريقة التي تناسب مستوى تركيزك الآن"</p>
          </div>

          <div className="grid md:grid-cols-3 gap-8">
            <Card 
              onClick={() => selectMode('exam-night')}
              className="p-12 glass border-indigo-500/30 hover:border-indigo-500 hover:scale-105 transition-all cursor-pointer group rounded-[60px] text-center space-y-8 relative overflow-hidden"
            >
            ...
            <Card 
              ...
              className="p-12 glass border-vermillion/30 hover:border-vermillion hover:scale-105 transition-all cursor-pointer group rounded-[60px] text-center space-y-8 relative overflow-hidden"
            >
              <div className="absolute top-0 right-0 w-32 h-32 bg-vermillion/10 rounded-bl-full -mr-16 -mt-16" />
            ...
            <Card 
              ...
              className="p-12 glass border-goldenrod/30 hover:border-goldenrod hover:scale-105 transition-all cursor-pointer group rounded-[60px] text-center space-y-8 relative overflow-hidden"
            >
              <div className="absolute top-0 right-0 w-32 h-32 bg-goldenrod/10 rounded-bl-full -mr-16 -mt-16" />
    ...
    return (
      <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in zoom-in duration-700 py-20 px-4 text-right" dir="rtl">
        <div className="text-center space-y-6">
          <div className="inline-block p-10 rounded-full bg-goldenrod/10 border-4 border-goldenrod/30 shadow-[0_0_80px_rgba(230,172,0,0.3)] mb-4">
            {percentage >= 90 ? <PartyPopper className="w-24 h-24 text-goldenrod" /> : <Trophy className="w-24 h-24 text-goldenrod" />}
          </div>
          <h1 className="text-8xl font-headline font-black text-white drop-shadow-2xl">
            {percentage >= 90 ? "أداء مذهل! 🔥" : "استمر في التقدم! 🚀"}
          </h1>
          <div className="flex flex-wrap justify-center gap-4">
            <Badge className="bg-goldenrod/20 text-goldenrod border-goldenrod/40 text-2xl px-10 py-3 rounded-full">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <Card className="p-12 text-center glass border-goldenrod/30 gold-glow rounded-[50px]">
            <p className="text-muted-foreground mb-4 font-bold text-xl uppercase tracking-widest">النسبة النهائية</p>
            <p className="text-7xl font-black text-goldenrod">{percentage}%</p>
          </Card>
          <Card className="p-12 text-center glass border-green-500/30 rounded-[50px]">
            <p className="text-muted-foreground mb-4 font-bold text-xl uppercase tracking-widest">إجابات صحيحة</p>
            <p className="text-7xl font-black text-green-500">{correct}</p>
          </Card>
          <Card className="p-12 text-center glass border-vermillion/30 rounded-[50px]">
            <p className="text-muted-foreground mb-4 font-bold text-xl uppercase tracking-widest">إجابات خاطئة</p>
            <p className="text-7xl font-black text-vermillion">{section.questions.length - correct}</p>
          </Card>
        </div>

        <div className="flex flex-col sm:flex-row gap-6 pt-10">
          <button onClick={() => window.location.reload()} className="flex-1 h-28 text-4xl font-black bg-goldenrod text-midnight rounded-[40px] hover:scale-[1.03] transition-all flex items-center justify-center shadow-2xl">
            <RotateCcw className="ml-4 w-10 h-10" /> إعادة المحاولة
          </button>
          <button onClick={onExit} className="flex-1 h-28 text-4xl font-black border-4 border-goldenrod text-goldenrod hover:bg-goldenrod/10 rounded-[40px] flex items-center justify-center transition-all">
            الخروج للرئيسية
          </button>
        </div>
      </div>
  ...
  return (
    <div className={cn(
      "max-w-5xl mx-auto space-y-12 animate-in slide-in-from-bottom-12 duration-700 py-16 pb-32 px-4 transition-all text-right relative",
      isTimeLow && !isFreeMode && "animate-shake",
    )} dir="rtl">
      
      {/* Dynamic Header */}
      <div className={cn(
        "flex justify-between items-center bg-midnight/80 p-8 rounded-[50px] border-2 border-white/5 backdrop-blur-3xl sticky top-6 z-50 shadow-[0_30px_60px_-15px_rgba(0,0,0,0.5)] transition-all",
        !isFreeMode && isTimeLow && "border-vermillion bg-vermillion/20 scale-[1.02]"
      )}>
        <Button onClick={onExit} variant="ghost" className="text-xl text-muted-foreground hover:text-white rounded-full font-bold px-8 h-14">
          <ChevronLeft className="ml-2" /> انسحاب
        </Button>

        {!isFreeMode ? (
          <div className={cn(
            "flex items-center gap-6 px-10 py-4 rounded-full border-2 transition-all",
            isTimeLow ? "bg-vermillion border-white text-white animate-pulse shadow-[0_0_30px_rgba(255,77,51,0.5)]" : "bg-white/5 border-white/10 text-white"
          )}>
            {isPressureMode ? <Flame className="w-8 h-8" /> : <Clock className="w-8 h-8" />}
            <span className="text-4xl font-black font-mono tracking-widest">{formatTime(timeLeft)}</span>
          </div>
        ) : (
          <Badge className="bg-white/5 px-10 py-4 rounded-full border border-white/10 text-muted-foreground text-2xl font-black">
            وضع التأمل الحر 🧘‍♂️
          </Badge>
        )}

        <div className="text-left font-black">
          <span className={cn("text-5xl text-goldenrod")}>
            {currentQuestionIndex + 1}
          </span>
          <span className="text-2xl text-muted-foreground opacity-50"> / {section.questions.length}</span>
        </div>
      </div>

      <Progress value={progress} className="h-5 rounded-full bg-white/5 border border-white/10 shadow-inner" />

      {/* Content Area */}
      <div className="space-y-12">
        {associatedPassage && (
          <Card className="p-12 glass rounded-[60px] border-2 border-goldenrod/20 shadow-2xl relative overflow-hidden">
            <div className="absolute top-0 right-0 w-4 h-full bg-goldenrod/20" />
             <div className="flex items-center gap-6 mb-10">
              <div className="p-4 bg-goldenrod/10 rounded-3xl">
                <BookOpen className="w-10 h-10 text-goldenrod" />
              </div>
              <h2 className="text-5xl font-black text-white">{associatedPassage.title}</h2>
            </div>
            <div className="text-3xl leading-[1.8] text-white/90 whitespace-pre-wrap font-medium">
        ...
        {currentGroup.map((q) => (
          <Card key={q.id} className={cn(
            "p-16 glass rounded-[80px] border-2 border-white/5 relative overflow-hidden group shadow-[0_40px_100px_rgba(0,0,0,0.5)]",
            ...
            <div className="flex justify-between items-start mb-16 gap-10">
              <h2 className="text-6xl font-headline font-black leading-[1.3] text-white flex-1">
                {q.question}
              </h2>
              <Button 
                variant="ghost" 
                size="icon" 
                className={cn(
                  "rounded-full w-20 h-20 transition-all shadow-xl",
                ...
                return (
                  <button
                    ...
                    className={cn(
                      ...
                      isSelected 
                        ? 'bg-goldenrod text-midnight border-white scale-[1.02] shadow-[0_20px_40px_rgba(230,172,0,0.4)]' 
                        : 'bg-white/5 text-white border-white/5 hover:border-goldenrod/40 hover:bg-white/10'
                    )}
                  >
                    <span className="opacity-50 ml-4">{['أ', 'ب', 'ج', 'د'][i]}.</span> {option}
                  </button>
                );
              })}
            </div>
      ...
      <div className="flex justify-center md:justify-end pt-16">
        <button 
          onClick={handleNext} 
          className="group relative px-24 py-12 rounded-[50px] text-5xl font-black bg-goldenrod text-midnight hover:scale-[1.05] transition-all shadow-[0_30px_60px_-10px_rgba(230,172,0,0.5)] overflow-hidden"
        >
